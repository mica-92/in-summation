...
<head>
    ...
    top: 4px;
        ...
        /* Brutalist Egg Tooltip */
        .popup-link:hover::before {{
            content: "\f7fb"; /* Unicode for fa-egg */
        ...
        #popup-image {{
            max-width: 100%;  /* Constrained by parent's max-width */
            max-height: 100%; /* Constrained by parent's max-height */
            object-fit: contain;
...
<div class="album-percentage total-minutes-value">{round(monthly_stats[month]['minutes'])} min</div>            </div>
    ...
    <div id="popup">
        <div style="position: relative;">
            <img id="popup-image" src="" alt="Popup Image" style="max-width: 100%; max-height: 90vh;">
            <button onclick="closePopup()" style="position: absolute; top: -15px; right: -15px; background: white; border: 3px solid black; width: 30px; height: 30px; font-weight: bold; cursor: pointer;">X</button>
        </div>
    </div>
...
<script>
    ...
    # Replace words with popups using the dictionary
    ee_dict = {
        "tis the damn season": "EE1.png",
        "Is It Over Now?": "EE2.png",
        "The Manuscript": "EE3.png",
        "august": "EE4.png",
        "Question...?": "EE5.png",
        "cardigan": "EE6.png",
        "the last great american dynasty": "EE7.png",
        "So High School": "EE8.png",
        "The Alchemy": "EE9.png",
        "You're Losing Me": "EE10.png",
        "Hey Stephen": "EE11.png",
        "marjorie": "EE12.png",
        "Back To December": "EE13.png",
        "High Infidelity": "EE14.png",
        "Gateaway Car": "EE15.png",
        "Cornelia Street": "EE16.png",
        "End Game": "EE17.png",
        "hi!!": "EE18.png",
        "Who's Afraid of Little Old Me?": "EE19.png",
        "Look What You Made Me Do": "EE20.png",
        "The Archer": "EE21.png",
        "Karma": "EE22.png",
        "cowboy like me": "EE23.png",
        "Style": "EE24.png",
        "Cassandra": "EE25.png",
        "You Belong With Me": "EE26.png",
        "Welcome to New York": "EE27.png",
        "Only The Young": "EE28.png",
        "Florida!!!": "EE29.png",
        "Wonderland": "EE30.png",
        "Crazier": "EE31.png",
    ...
    # Create a regex pattern that matches any of the dictionary keys as whole words
    pattern = re.compile(r'\b(' + '|'.join(re.escape(key) for key in ee_dict.keys()) + r')\b')
    
    # Replacement function that creates the popup link
    def replace_match(match):
        word = match.group(0)
        image_path = ee_dict.get(word)
        if image_path:
            return f'<a href="#" onclick="openPopup(\'{image_path}\'); return false;" class="popup-link" title="Click!">{word}</a>'      
    
    # Replace all occurrences in the HTML (except within script and style tags)
    parts = []
    last_end = 0
    for match in re.finditer(r'<(script|style).*?>.*?</\1>', html, re.DOTALL):
        # Process text before the script/style tag
        parts.append(pattern.sub(replace_match, html[last_end:match.start()]))
        # Add the script/style tag unchanged
        parts.append(html[match.start():match.end()])
        last_end = match.end()
    
    # Process the remaining part after the last script/style tag
    parts.append(pattern.sub(replace_match, html[last_end:]))
...
def replace_words_with_popups(html_content, popup_dict):
    """
    Replace words in HTML content with pop-up links based on a dictionary.
    
    Args:
        html_content (str): The HTML content to process
        popup_dict (dict): Dictionary mapping words to image paths
        
    Returns:
        str: Modified HTML with pop-up links
    """
    # Create a regex pattern that matches any of the dictionary keys as whole words
    pattern = re.compile(r'\b(' + '|'.join(re.escape(key) for key in popup_dict.keys()) + r')\b')
    
    # Replacement function that creates the popup link
    def replace_match(match):
        word = match.group(0)
        image_path = popup_dict.get(word)
        if image_path:
            return f'<a href="#" onclick="openPopup(\'{image_path}\'); return false;" class="popup-link">{word}</a>'
        return word
    
    # Replace all occurrences in the HTML (except within script and style tags)
    parts = []
    last_end = 0
    for match in re.finditer(r'<(script|style).*?>.*?</\1>', html_content, re.DOTALL):
        # Process text before the script/style tag
        parts.append(pattern.sub(replace_match, html_content[last_end:match.start()]))
        # Add the script/style tag unchanged
        parts.append(html_content[match.start():match.end()])
        last_end = match.end()
    
    # Process the remaining part after the last script/style tag
    parts.append(pattern.sub(replace_match, html_content[last_end:]))
